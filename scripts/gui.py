...
class SleapProcessor:
    ...
    def process_video_file(
        ...
        if  Path.is_file(Path(output_h5_path)):
         render_command = (
            f"sleap-render -o {output_mp4_path} -f 50 {output_h5_path}"
         )
         print(f"Rendering video: {render_command}")
         self.handle_subprocess(render_command)
        else:
            print(f"cannot render video, file does not exist: {output_h5_path} ")
...
class InputBox(QWidget):
    ...
    def initUI(self):
        ...
        label = QLabel(self)
        pixmap = QPixmap(r"\\gpfs.corp.brain.mpg.de\stem\data\project_hierarchy\code\sleap\logo.jpg")  # replace 'image.jpg' with your image file path
...
class InputBox(QWidget):
    ...
    def initUI(self):
        ...
        layout.addWidget(self.cb)
        # self.cb = QComboBox()
        # self.cb.addItem("mouse")
        # self.cb.addItem("cricket")
        # self.cb.addItem("pups")
        # self.cb.addItem(["mouse","cricket"])
        # self.cb.addItem(["mouse","pups"])
        # self.cb.addItem(["mouse","cricket","pups"])
...
#         # Add image at the top
#         label = QLabel(self)
#         pixmap = QPixmap('image.jpg')  # replace 'image.jpg' with your image file path
#         label.setPixmap(pixmap)
#         layout.addWidget(label)
