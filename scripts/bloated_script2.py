...
#os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2' #supresses an annoying warning about loop optimization that doesn't affect running
#fix  warning about loop optimization that doesn't affect running
...
class InputBox(QWidget):
    ...
    def initUI(self):
        ...
        # self.btn_file.clicked.connect(lambda: self.getfile(False))
        # file_path_layout.addWidget(self.btn_file)
        # self.btn_dir = QPushButton("Browse Directory")
        # self.btn_dir.clicked.connect(lambda: self.getfile(True))
        # file_path_layout.addWidget(self.btn_dir)
    ...
    def getfile(self):
        ...
        fname = QFileDialog.getOpenFileName(self, 'Open File or Directory', '\\\\gpfs.corp.brain.mpg.de\\stem\\data\\project_hierarch', 'All Files (*);;Directories (*)', options=options)
        if fname[0]:
            self.le.setText(fname[0])
        
    # def getfile(self, is_directory):
    #  if is_directory:
    #     fname = QFileDialog.getExistingDirectory(self, 'Open directory', '\\\\gpfs.corp.brain.mpg.de\\stem\\data\\project_hierarch')  # Set the default path
    #  else:
    #     fname = QFileDialog.getOpenFileName(self, 'Open file', '\\\\gpfs.corp.brain.mpg.de\\stem\\data\\project_hierarch')  # Set the default path
    #  self.le.setText(fname[0])
