...
class InputBox(QWidget):    
    ...
    def __init__(self):
        ...
        self.file_path = Path()
        self.animal_type = []
        self.csv_path = Path(r"\\gpfs.corp.brain.mpg.de\stem\\data\project_hierarchy\Sleap_projects\sleap_model_paths.csv").as_posix()
        self.optional_args = ''
...
#        grid_layout = QGridLayout()
        ...
        self.csvbtn = QPushButton("Browse for CSV file")
    ...
    #     """
    #     fileName, _ = QFileDialog.getOpenFileName(self, "select video file", "", "All Files (*);;MP4 Files (*.mp4);;avi Files (*.avi)")
    #     if fileName:
    ...
    #          #self.save_config(csv_path=str(self.csv_path))
    ...
    def get_save_csv_file(self):
         ...
         Updates the CSV path in both the GUI and config file.
         """
         fileName, _ = QFileDialog.getSaveFileName(self, "QFileDialog.getSaveFileName()", "", "CSV Files (*.csv)")
         if fileName:
             self.csv_path = Path(fileName)
             # Update the GUI text field
             self.model_path_CSV_le.setText(str(self.csv_path))
             # Save the new CSV path to the config file
             self.save_config(csv_path=str(self.csv_path))
