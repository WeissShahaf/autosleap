...
class SleapProcessor:
    ...
    def read_config(self):
     ...
     if self.config_path.exists() and os.path.getsize(self.config_path) > 0:
        ...
        # if self.config_path and self.config_path.exists() and os.path.getsize(self.config_path) > 0:
        #     with open(self.config_path, 'r') as f:
        #         data = json.load(f)

        #     self.file_path = self.convert(data['file_path'], 'path')
        #     self.animal_type = data['animal_type']
        #     self.csv_path = self.convert(data['csv_path'], 'path')
        #     self.optional_args = data['optional_args']
        #     self.paths_csv = self.convert(data['csv_path'], 'path')
        #     self.log_file_path = self.convert(data.get('log_file_path', 'tracked/sleap_commands.log'), 'path')
        #     self.chosen_model = self.convert(data['chosen_model'], 'path')
        #     self.model_prefix = data['model_prefix']
        # else:
        #     raise FileNotFoundError("Configuration file not found or is empty.")
    ...
    def find_model_path_from_csv(self, model_type,csv_path=None):
      ...
      #  self.update_status_callback(PureWindowsPath(current_path))
    ...
    def handle_subprocess(self, command):
        ...
        self.update_status_callback(out)  # Call the callback function
    ...
    def process_video_file(self, video_file, input_folder, output_folder, model_type, model_path):
        ...
        #return text  # Return the text instead of printing it
     
    def process_and_print(self, video_file, input_folder, output_folder, model_type, model_path):
        out=f"Processing: {video_file}"
        self.update_status_callback(out)
...
#            if num_processes > len(video_files): num_processes = len(video_files)
    ...
    def run_sleap(self):
        ...
        self.update_status_callback(model_types)
...
#        IPython.core.debugger.set_trace()           
        ...
        self.update_status_callback(video_files)
        for model_type in model_types:
            self.update_status_callback(model_type)
            ...
            return 'finished running all files'
    # # Get exception info
