...
#        grid_layout = QGridLayout()
    ...
    def update_combos(self, useCSV=False,csvfile=None):
        ...
        if useCSV:
            ...
            if csvfile is not None:
                ...
                csvfile = Path(self.model_path_CSV_le.text()).as_posix()
    ...
    def run_sleapGUI(self):
        ...
        try:          
            file_path = Path(self.le.text()).as_posix()    
            csv_path = Path(self.model_path_CSV_le.text()).as_posix()
            manual_model_path = Path(self.model_path_le.text()).as_posix()
            
            
            if self.manual_model_checkbox.isChecked():
                chosen_model = Path(manual_model_path).as_posix()
...
#            sleap_processor = SleapProcessor(self.update_status_message)
            ...
            directory = Path(os.path.dirname(file_path)).as_posix() / 'tracked'        
